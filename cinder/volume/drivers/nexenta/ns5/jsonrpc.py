...
def check_error(response):
    ...
    if code not in (200, 201, 202):
        ...
        try:
            ...
            msg = (_('Could not parse response: %(code)s %(reason)s %(body)s'),
                   {'code': code,
                    'reason': reason,
                    'body': body})
        ...
        msg = (_('Got bad response: %(code)s %(reason)s %(content)s'),
               {'code': code,
                'reason': reason,
                'content': content})
        raise exception.NexentaException(msg)
...
class RESTCaller(object):
    ...
    def __call__(self, *args):
        ...
        LOG.debug('Issuing call to NS: %(method)s %(url)s, data: %(data)s',
        ...
        try:
            ...
            LOG.warning('Connection error on call to NS: %(method)s '
                        '%(url)s, data: %(data)s',
                        {'method': self.__method,
        ...
        try:
            ...
            if exc.kwargs['message']['code'] == 'ENOENT':
                LOG.warning('Exception on call to NexentaStor: '
        ...
        if response.status_code == 202 and content:
            ...
            while keep_going:
                ...
                try:
                    ...
                    if exc.kwargs['message']['code'] == 'ENOENT':
                        LOG.debug('Exception on call to NexentaStor: '
                                  '%(method)s %(url)s data: %(data)s '
                                  'returned message: %(message)s',
                                  {'method': self.__method,
                                   'url': url,
    ...
    def handle_failover(self):
        if self.__proxy.backup:
            LOG.info('Primary host %(host)s is unavailable, '
...
class HTTPSAuth(requests.auth.AuthBase):
    ...
    def https_auth(self):
        LOG.debug('Sending auth request to %(url)s',
                  {'url': self.url})
        ...
        content = json.loads(response.content) if response.content else None
        LOG.debug('Auth response: %(code)s %(reason)s %(content)s',
                  {'code': response.status_code,
                   'reason': response.reason,
                   'content': content})
        check_error(response)
        ...
        if response.content:
            ...
            del content['token']
            return token
        msg = (_('Got bad response: %(code)s %(reason)s'),
               {'code': response.status_code,
                'reason': response.reason})
        raise exception.NexentaException(msg)
